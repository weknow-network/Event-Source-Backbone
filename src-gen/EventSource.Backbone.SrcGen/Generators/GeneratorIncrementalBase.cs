...
using EventSource.Backbone.SrcGen.Generators.Entities;

using static EventSource.Backbone.Helper;
...
namespace EventSource.Backbone
{
    ...
    internal abstract class GeneratorIncrementalBase : IIncrementalGenerator
    {
        ...
        private static readonly string[] DEFAULT_USING = new[]{
                            ...
                            "EventSource.Backbone",
                            "EventSource.Backbone.Building" }.Select(u => $"using {u};").ToArray();
                            ...
                            SyntaxReceiverResult info)
        {
            ...
            foreach (var (fileName, content, dynamicNs, usn) in codes)
            {
                ...
                builder.AppendLine($"namespace {overrideNS ?? "EventSource.Backbone"}");
                builder.AppendLine("{");
